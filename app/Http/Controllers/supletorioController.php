...
class supletorioController extends Controller
{
    public function listar()
    {
        ...
        ->whereHas('documento.tipoDocumentoDetalle.tipoDocumento', function($query){
            $query->where('Nombre_tipo','CERTIFICADOS SUPLETORIO');
        })
        ...
        ->map(function ($diploma) {
            return [
                ...
                'tipo_documento'       => 'CERTIFICADOS SUPLETORIO',  // Valor fijo
                'documento_id'         => $diploma->documento_id,
                'textos_id'            => $diploma->documento->textos->pluck('id')->toArray(),
            ];
        });

        return Inertia::render('Supletorio/Listar', [
            'documentos' => $diplomas
        ]);
    }
}
