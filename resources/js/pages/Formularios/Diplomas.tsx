...
import FloatingDate from '@/components/propios/FloatingDate';
...
export default function DiplomasFormulario(props: DocumentosFormularioProps) {
    ...
    const handleGuardar = async () => {
        ...
        datos.append('carrera', formData.carrera);
        datos.append('fecha_emision', formData.fecha_emision);
        datos.append('fecha_nacimiento', formData.fecha_nacimiento);
        ...
        try {
            await axios.post('/formulario/diplomas/guardar', datos, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
    ...
    const mostrarCarrera = tipoSeleccionado?.Nombre_tipo !== 'DIPLOMAS DE BACHILLER';
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Diplomas', href: '/formulario/diplomas' }]}>
            <Head title="Formulario de diplomas" />
            {alert && <CustomAlert message={alert.message} type={alert.type} onClose={() => setAlert(null)} />}

            <div className="space-y-6 p-6">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Formulario de diplomas</h1>
                ...
                <form onSubmit={handlePreprocesar} className="dark:bg-sidebar space-y-4 rounded-xl bg-white p-6 shadow-md">
                    ...
                    <div className="grid grid-flow-row-dense grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <FloatingInput
                            ...
                            onChange={handleChange}
                            required
                            error={errors.nombre ? errors.nombre[0] : ''}
                        />
                        <FloatingInput
                            id="apellido"
                            name="apellido"
                            ...
                            onChange={handleChange}
                            required
                            error={errors.apellido ? errors.apellido[0] : ''} // Pasamos el error si existe
                        />
                        {mostrarCarrera && (
                            <FloatingInput
                                id="carrera"
                                ...
                                onChange={handleChange}
                                required
                                error={errors.carrera ? errors.carrera[0] : ''} // Pasamos el error si existe
                            />
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <FloatingDate
                            ...
                            onChange={handleChange}
                            required
                            error={errors.fecha_nacimiento ? errors.fecha_nacimiento[0] : ''}
                        />
                        <FloatingDate
                            id="fecha_emision"
                            name="fecha_emision"
                            ...
                            onChange={handleChange}
                            required
                            error={errors.fecha_emision ? errors.fecha_emision[0] : ''}
                        />
                    </div>
                    {errors.fecha_emision && <p className="mt-1 text-sm text-red-500">{errors.fecha_emision[0]}</p>}
